...
const EditCourse = ({ params }) => {
  ...
  const handleDeleteCourse = () => {
    const token = localStorage.getItem("JWT");
    axios
      .delete("http://localhost:8080" + `/api/v1/course/${params?.id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const handleAddNewChapter = (newChapter, handleSaveChapter) => {
    if (newChapter.length) {
      ...
      axios
        .post(
          "http://localhost:8080" +
            `/api/v1/chapter/create?courseId=${params?.id}`,
          data,
          {
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const handleDeleteChapter = (indexChapter) => {
    ...
    axios
      .delete(
        "http://localhost:8080" +
          `/api/v1/chapter/delete?courseId=${params?.id}&chapter=${indexChapter}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      )
      .then((responses) => {
        if (
          ...
          responses.data.message === "delete chapter successfully"
        ) {
    ...
    setPrevChapterName(
      prevChapterName.filter((course, index) => index != indexChapter)
    );
  };
  ...
  const handleEditChapter = (indexChapter) => {
    ...
    axios
      .put(
        "http://localhost:8080" +
          `/api/v1/chapter/update?courseId=${params?.id}&chapter=${indexChapter}`,
        data,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      .then((responses) => {
        if (
          responses.status === 200 &&
          responses.data.message === "update chapter successfully"
        ) {
        ...
        <div className="w-full">
          <div className="relative w-full mt-4 flex-between border-b border-solid border-black">
            ...
            <div className="w-1/2">
              {confirmDelete ? (
                <button
                  className="float-right medium-orange-button"
                ...
                <button
                  className="float-right medium-red-button"
